...
(function () {
  ...
  function loadTargetHistoryStore() {
    try {
      const value = typeof GM_getValue === 'function' ? GM_getValue(TARGET_HISTORY_KEY, {}) : {};
      return normalizeTargetHistory(value);
    } catch (error) {
      console.warn('[nbce] failed to load target history', error);
      return {};
    }
  }

  function saveTargetHistoryStore(nextStore) {
    if (typeof GM_setValue === 'function') {
      return Promise.resolve(GM_setValue(TARGET_HISTORY_KEY, nextStore));
    }
    return Promise.resolve();
  }

  function normalizeTargetHistoryUrl(value) {
  ...
  function normalizeTargetHistoryEntry(entry) {
    const source = isRecord(entry) ? entry : Array.isArray(entry) ? { recent: entry } : {};
    const pinned = normalizeTargetHistoryUrl(source.pinned || '');
    ...
    recentSource.forEach((value) => {
      const normalized = normalizeTargetHistoryUrl(value);
    ...
    return {
      pinned,
      recent: recent.slice(0, TARGET_HISTORY_LIMIT),
    };
  }

  function normalizeTargetHistory(value) {
    if (!isRecord(value)) {
      return {};
    }
    const nextStore = {};
    Object.entries(value).forEach(([origin, entry]) => {
      nextStore[origin] = normalizeTargetHistoryEntry(entry);
    });
    return nextStore;
  }

  function getTargetHistoryForOrigin(origin = location.origin, store = loadTargetHistoryStore()) {
    return normalizeTargetHistoryEntry(store[origin] || null);
  }

  function upsertTargetHistory(history, origin, targetUrl) {
    const nextStore = normalizeTargetHistory(history);
    const originKey = origin || location.origin;
    const entry = normalizeTargetHistoryEntry(nextStore[originKey]);
    const nextUrl = normalizeTargetHistoryUrl(targetUrl);
    if (!nextUrl) {
      nextStore[originKey] = entry;
      return nextStore;
    }
    const recent =
      nextUrl === entry.pinned
        ? entry.recent
        : [nextUrl, ...entry.recent.filter((value) => value !== nextUrl)].slice(0, TARGET_HISTORY_LIMIT);
    nextStore[originKey] = {
      pinned: entry.pinned,
      recent,
    };
  ...
  function pinTargetHistory(history, origin, targetUrl) {
    const nextStore = normalizeTargetHistory(history);
    const originKey = origin || location.origin;
    const entry = normalizeTargetHistoryEntry(nextStore[originKey]);
    const nextUrl = normalizeTargetHistoryUrl(targetUrl);
    ...
    const demotedPinned = entry.pinned && entry.pinned !== nextUrl ? [entry.pinned] : [];
    nextStore[originKey] = {
      pinned: nextUrl,
      recent: [
        ...demotedPinned,
        ...entry.recent.filter((value) => value !== nextUrl && value !== entry.pinned),
      ].slice(0, TARGET_HISTORY_LIMIT),
    };
    return nextStore;
  }

  function removeTargetHistory(history, origin, targetUrl) {
    const nextStore = normalizeTargetHistory(history);
    const originKey = origin || location.origin;
    const entry = normalizeTargetHistoryEntry(nextStore[originKey]);
    const nextUrl = normalizeTargetHistoryUrl(targetUrl);
    ...
    const pinned = entry.pinned === nextUrl ? '' : entry.pinned;
    const recent = entry.recent.filter((value) => value !== nextUrl);
    if (!pinned && !recent.length) {
      delete nextStore[originKey];
      return nextStore;
    }
    nextStore[originKey] = {
      pinned,
  ...
  async function updateTargetHistoryStore(mutator) {
    const currentStore = loadTargetHistoryStore();
    const nextStore = normalizeTargetHistory(typeof mutator === 'function' ? mutator(currentStore) || currentStore : currentStore);
    await saveTargetHistoryStore(nextStore);
    return nextStore;
  }
  ...
  function captureDebugResponseText(value) {
    ...
    return {
      ...
      truncated: true,
      omittedChars: text.length - DEBUG_RESPONSE_TEXT_LIMIT,
      kind: 'text',
      length: text.length,
    };
  }

  function summarizeDebugBodyMeta(body) {
    ...
    if (body.kind === 'text') {
      ...
      return {
        summary: truncateDebugText(text),
        meta: {
          kind: 'text',
          truncated: text.length > DEBUG_TEXT_LIMIT,
          omittedChars: Math.max(0, text.length - DEBUG_TEXT_LIMIT),
          length: text.length,
        },
      };
    }
    if (body.kind === 'formData' && typeof body.value?.forEach === 'function') {
      ...
      body.value.forEach((value, key) => {
        ...
        const text = `${value ?? ''}`;
        const truncatedText = truncateDebugText(text, 240);
        if (truncatedText !== text) {
          anyTruncated = true;
          omittedChars += Math.max(0, text.length - 240);
        }
        entries.push(`${key}: ${truncatedText}`);
      });
      const summaryText = entries.join('\n');
      const truncatedSummary = truncateDebugText(summaryText);
      return {
        summary: truncatedSummary,
        meta: {
          kind: 'formData',
          truncated: anyTruncated || truncatedSummary !== summaryText,
          omittedChars: omittedChars + Math.max(0, summaryText.length - DEBUG_TEXT_LIMIT),
          length: summaryText.length,
        },
      };
    }
    if (body.kind === 'blob') {
      const byteLength = Number(body.value?.size) || 0;
      return {
        summary: `[Blob ${byteLength} bytes]`,
        meta: {
          kind: 'blob',
          truncated: false,
          omittedChars: 0,
    ...
    if (body.kind === 'arrayBuffer') {
      const byteLength = Number(body.value?.byteLength) || 0;
      return {
        summary: `[ArrayBuffer ${byteLength} bytes]`,
        meta: {
          kind: 'arrayBuffer',
          truncated: false,
          omittedChars: 0,
    ...
    const originalText = `${body.value ?? ''}`;
    const text = truncateDebugText(originalText);
    return {
      summary: text,
      meta: {
        kind: body.kind || 'unknown',
        truncated: originalText.length > DEBUG_TEXT_LIMIT,
        omittedChars: Math.max(0, originalText.length - DEBUG_TEXT_LIMIT),
  ...
  function addDebugLog(entry) {
    const requestBodyMeta = normalizeDebugBodyMeta(entry.requestBodyMeta);
    const responseMeta = normalizeDebugBodyMeta(entry.responseMeta);
    const nextEntry = {
  ...
  function formatDebugDiagnostics(diagnostics, limit = 8) {
  ...
  function normalizeDebugBodyMeta(meta) {
    if (!isRecord(meta)) {
      return {
        kind: 'none',
        truncated: false,
    ...
    return {
      ...
      omittedChars: Number(meta.omittedChars) || 0,
    };
  }

  function deriveDebugLogStatus(log) {
    const status = Number(log?.status) || 0;
    const errorText = `${log?.error || ''}`.trim();
    const transportReason = [
      ['timeout', /timeout|超时/i],
      ['abort', /abort|cancel|取消/i],
      ['unsupported', /unsupported|不支持/i],
      ['network', /network|请求失败|失败/i],
    ].find(([, pattern]) => pattern.test(errorText))?.[0];
    const isTransportError = Boolean(status === 0 || transportReason || (!status && log?.ok === false));
    const isHttpError = status >= 400;
    const isSuccessfulResponse = !isTransportError && status >= 200 && status < 400;
    return {
      status,
      statusText: `${log?.statusText || ''}`.trim(),
      errorText,
      transportReason: transportReason || '',
      isTransportError,
      isHttpError,
      isSuccessfulResponse,
      isFailure: !isSuccessfulResponse,
    };
  }

  function buildDebugLogViewModels(logs, context = {}) {
    const debugRules = isRecord(context.debugRules) ? context.debugRules : {};
    const analyzeRequest = typeof context.analyzeRequest === 'function' ? context.analyzeRequest : analyzeNocoBaseRequest;
    const requestLogs = Array.isArray(logs) ? logs : [];

    return requestLogs.map((log) => {
      const semantic = analyzeRequest(log) || {};
      const statusInfo = deriveDebugLogStatus(log);
      const diagnostics = Array.isArray(log?.diagnostics) ? log.diagnostics : [];
      const ruleKey = buildDebugRuleKey(log?.method, log?.endpoint);
      const overrideRule = log?.endpoint ? debugRules[ruleKey] || null : null;
      const hasOverrideRule = Boolean(overrideRule?.enabled);
      const isOverridden = Boolean(log?.overridden);
      const diagnosticsText = formatDebugDiagnostics(diagnostics);
      const searchText = [
        log?.method,
        log?.endpoint,
        log?.url,
      ...
      return {
        ...log,
        requestBodyMeta: normalizeDebugBodyMeta(log?.requestBodyMeta),
        semanticSummary: semantic.summary || '',
        semanticFilterText: semantic.filterText || '',
        hasOverrideRule,
        isOverridden,
        overrideRule,
        diagnosticsText,
  ...
  function filterDebugLogViewModels(items, filters = {}) {
    const requestItems = Array.isArray(items) ? items : [];
    const requestTotal = requestItems.length;
    const keyword = `${filters.keyword || ''}`.trim().toLowerCase();
    const methodFilter = `${filters.method || ''}`.trim().toUpperCase();
    const statusFilter = `${filters.statusFilter || 'all'}`.trim().toLowerCase();
    const overrideFilter = `${filters.overrideFilter || 'all'}`.trim().toLowerCase();
    const diagnosticsOnly = Boolean(filters.diagnosticsOnly);
    const resourceOnly = Boolean(filters.resourceOnly);
    ...
    const filtered = requestItems.filter((item) => {
      ...
      if (methodFilter && methodFilter !== 'ALL' && `${item.method || ''}`.toUpperCase() !== methodFilter) {
        return false;
      }
      if (statusFilter === 'success' && !item.isSuccessfulResponse) {
        return false;
      }
      if (statusFilter === 'failure' && item.isSuccessfulResponse) {
        return false;
      }
      if (overrideFilter === 'overridden' && !item.isOverridden) {
        return false;
      }
      if (overrideFilter === 'normal' && item.isOverridden) {
        return false;
      }
      if (diagnosticsOnly && !(Array.isArray(item.diagnostics) && item.diagnostics.length)) {
        return false;
      }
      return true;
    });
    ...
    return {
      items: filtered,
      counts: {
        requestHits: filtered.length,
        requestTotal,
      },
    };
  }

  function filterResourceIssues(issues, filters = {}) {
    const resourceItems = Array.isArray(issues) ? issues : [];
    const resourceTotal = resourceItems.length;
    const keyword = `${filters.keyword || ''}`.trim().toLowerCase();
    ...
    return {
      items: filtered,
      counts: {
        resourceHits: filtered.length,
        resourceTotal,
      },
    };
  }

  function resolveVisibleDebugSelection(selectedDebugLogId, visibleItems, resourceOnly) {
    ...
    if (!selectedDebugLogId && items.length) {
      return {
        selectedDebugLogId: items[0].id,
        selectedLog: items[0],
        shouldClearEditor: false,
      };
    }
    const selectedLog = items.find((item) => item.id === selectedDebugLogId) || null;
    if (!selectedLog) {
      return {
        selectedDebugLogId: '',
        selectedLog: null,
  ...
  function shortPanelText(value, fallback = '-') {
    const text = `${value || ''}`.trim();
    if (!text) {
      return fallback;
    }
    try {
      ...
      const pathname = normalizeRelativePath(url.pathname).replace(/\/$/g, '');
  ...
  function buildPanelStatusSummary(rule, state, counts) {
    const requestCountText = `${counts?.requestHits || 0}/${counts?.requestTotal || 0}`;
    const resourceCountText = `${counts?.resourceHits || 0}/${counts?.resourceTotal || 0}`;
    const targetText = rule?.enabled ? shortPanelText(rule.targetEntryUrl) : '未解析';
    const appText = rule?.enabled && rule.targetAppName ? rule.targetAppName : '主应用';
    const badgeItems = [];

    badgeItems.push({
      text: rule?.enabled ? 'A->B' : 'No rule',
      tone: rule?.enabled ? 'success' : 'warning',
    });
    badgeItems.push({
      text: state?.debugEnabled ? 'Debug' : 'No debug',
      tone: state?.debugEnabled ? 'info' : 'muted',
    });
    if (!state?.isLikelyNocoBasePage) {
      badgeItems.push({
        text: 'Non-NB',
        tone: 'warning',
      });
    ...
    return {
      main: `A ${shortPanelText(location.origin)} · B ${targetText} · App ${appText}`,
      secondary: `Debug ${state?.debugEnabled ? 'On' : 'Off'} · Request ${requestCountText} · Resource ${resourceCountText}`,
      badgeItems,
      requestCountText,
      resourceCountText,
    };
  ...
  function formatDebugCopyResponseSection(kind, value, meta = {}) {
    const text = `${value || ''}`;
    const lines = [];
    lines.push(`${kind} =`);
    if (meta.source) {
    ...
    if (Number.isFinite(meta.omittedChars) && meta.omittedChars > 0) {
      lines.push(`  omittedChars: ${meta.omittedChars}`);
    }
    if (text) {
      lines.push(`  value =`);
      text.split('\n').forEach((line) => {
  ...
  function buildDebugCopyPayload(log, kind, context = {}) {
    const selectedLog = isRecord(log) ? log : {};
    const semantic = context.semantic || analyzeNocoBaseRequest(selectedLog);
    const diagnostics = Array.isArray(context.diagnostics) ? context.diagnostics : selectedLog.diagnostics || [];
    const rule = context.overrideRule || selectedLog.overrideRule || null;
    const editorValue = `${context.editorValue ?? ''}`;
    const capturedResponse = `${selectedLog.responseText ?? ''}`;
    const hasOverrideRule = typeof rule?.responseText === 'string';
    const overrideResponse = hasOverrideRule ? `${rule.responseText || ''}` : '';
    const responseMeta = normalizeDebugBodyMeta(selectedLog.responseMeta);
    const capturedSection = formatDebugCopyResponseSection('capturedResponse', capturedResponse, {
      source: 'captured',
      kind: 'captured',
      truncated: Boolean(selectedLog.responseTruncated || responseMeta.truncated),
      omittedChars: Number.isFinite(responseMeta.omittedChars) && responseMeta.omittedChars > 0 ? responseMeta.omittedChars : 0,
    });
    const overrideSection = hasOverrideRule
      ? formatDebugCopyResponseSection('overrideResponse', overrideResponse, {
          source: 'overrideRule',
          kind: 'override',
          truncated: false,
          omittedChars: 0,
      ...
      : '';
    const editedDiffersFromCaptured = editorValue !== capturedResponse;
    const editedDiffersFromOverride = editorValue !== overrideResponse;
    const editedSection =
      editorValue && editedDiffersFromCaptured && (!hasOverrideRule || editedDiffersFromOverride)
        ? formatDebugCopyResponseSection('editedResponse', editorValue, {
            source: 'editor',
            kind: 'edited',
            truncated: false,
            omittedChars: 0,
    ...
    if (kind === 'endpoint') {
      return `${selectedLog.method || 'GET'} ${selectedLog.endpoint || selectedLog.url || ''}`.trim();
    }
    if (kind === 'semantic') {
      return semantic.summary || '未识别到 NocoBase collection/action 语义';
    }
    if (kind === 'diagnostics') {
      return formatDebugDiagnostics(diagnostics);
    }

    const requestBodyMeta = normalizeDebugBodyMeta(selectedLog.requestBodyMeta);
    const lines = [
      'method = ' + `${selectedLog.method || 'GET'}`.toUpperCase(),
      'endpoint = ' + `${selectedLog.endpoint || ''}`,
      'url = ' + `${selectedLog.url || ''}`,
      `status = ${selectedLog.status || 0}${selectedLog.statusText ? ` ${selectedLog.statusText}` : ''}`.trim(),
      `durationMs = ${Number(selectedLog.durationMs) || 0}`,
      `semanticSummary = ${semantic.summary || '未识别到 NocoBase collection/action 语义'}`,
      'diagnostics =',
      ...(Array.isArray(diagnostics) && diagnostics.length
        ? diagnostics.flatMap((diagnostic) => [
            `  [${diagnostic.level || 'info'}] ${diagnostic.message || ''}`,
            diagnostic.url ? `  url: ${diagnostic.url}` : '',
            diagnostic.detail ? `  detail: ${diagnostic.detail}` : '',
          ])
        : ['  (empty)']),
      'requestBody =',
      `  kind: ${requestBodyMeta.kind || 'none'}`,
      `  truncated: ${requestBodyMeta.truncated ? 'true' : 'false'}`,
      `  omittedChars: ${Number(requestBodyMeta.omittedChars) || 0}`,
      '  summary =',
      ...(selectedLog.requestBodySummary
        ? `${selectedLog.requestBodySummary}`.split('\n').map((line) => `    ${line}`)
        : ['    (empty)']),
      capturedSection,
    ];
    if (overrideSection) {
      lines.push(overrideSection);
    }
    if (editedSection) {
      lines.push(editedSection);
    }
    return lines.filter(Boolean).join('\n');
  }

  function getDebugCopyPayloadText(log, kind, context = {}) {
    return buildDebugCopyPayload(log, kind, context);
  }
...
//# sourceURL=nbce-page-bootstrap.js`;
  ...
  function handleBridgeRequest() {
    ...
    const { summary: requestBodySummary, meta: requestBodyMeta } = summarizeDebugBodyMeta(payload.body);
  ...
  function ensurePanel(openPanel) {
    ...
    const state = {
      ...
      targetHistoryStore: loadTargetHistoryStore(),
    };
    ...
    style.textContent = `
      ...
      .nbce-statusbar-badges {
        display: flex;
        flex-wrap: wrap;
        gap: 4px;
      }
      .nbce-statusbar-badge {
        display: inline-flex;
        align-items: center;
        padding: 1px 6px;
        border-radius: 999px;
      ...
      .nbce-statusbar-badge.success {
        background: #dcfce7;
        color: #166534;
        border-color: rgba(22, 101, 52, 0.14);
      }
      .nbce-statusbar-badge.warning {
        background: #fef3c7;
        color: #92400e;
        border-color: rgba(146, 64, 14, 0.14);
      }
      .nbce-statusbar-badge.info {
        background: #dbeafe;
        color: #1d4ed8;
        border-color: rgba(29, 78, 216, 0.14);
      }
      .nbce-statusbar-badge.muted {
        background: #e2e8f0;
        color: #334155;
      ...
      .nbce-history-chip:hover {
        background: #f8fafc;
      }
      .nbce-history-chip.pinned {
        border-color: rgba(14, 165, 233, 0.38);
        background: #ecfeff;
      }
      .nbce-history-chip.active {
        border-color: rgba(15, 23, 42, 0.42);
        background: #e0f2fe;
      }
      .nbce-history-actions {
        display: flex;
        gap: 4px;
      }
      .nbce-icon-button {
      ...
      .nbce-debug-filters {
        display: grid;
        grid-template-columns: minmax(0, 1fr) repeat(4, minmax(0, 120px));
        gap: 6px;
        margin-bottom: 10px;
      }
      .nbce-debug-filter,
      ...
      .nbce-debug-filter:focus,
      .nbce-debug-select:focus {
        border-color: rgba(14, 165, 233, 0.78);
        box-shadow: 0 0 0 3px rgba(14, 165, 233, 0.12);
      }
      .nbce-debug-filter-toggle {
        display: inline-flex;
        align-items: center;
        gap: 6px;
        min-height: 34px;
      ...
      .nbce-debug-filter-toggle input {
        margin: 0;
      }
      .nbce-button {
        appearance: none;
      ...
      .nbce-debug-resource-list {
        display: grid;
        gap: 6px;
        max-height: 430px;
        overflow: auto;
      }
      .nbce-debug-resource-item {
        border: 1px solid rgba(148, 163, 184, 0.22);
        border-radius: 6px;
        background: #ffffff;
        padding: 8px 10px;
      ...
      .nbce-debug-resource-item .nbce-debug-resource-title {
        font-weight: 700;
      }
      .nbce-debug-resource-item .nbce-debug-resource-body {
        margin-top: 4px;
        color: #475569;
        white-space: pre-wrap;
    ...
    shell.innerHTML = `
      ...
      <div class="nbce-card" ${state.expanded ? '' : 'hidden'}>
        <div class="nbce-header">
          ...
          <div class="nbce-statusbar">
            <div class="nbce-statusbar-line">
              <span class="nbce-statusbar-main"></span>
            </div>
            <div class="nbce-statusbar-line">
              <span class="nbce-statusbar-secondary"></span>
              <span class="nbce-statusbar-badges"></span>
            </div>
          </div>
        </div>
        <div class="nbce-body">
          ...
          <div class="nbce-pane nbce-config-pane" data-pane="config">
            ...
            <div class="nbce-field">
              ...
              <div class="nbce-history">
                <div class="nbce-history-head">
                  <span>最近使用</span>
                  <div class="nbce-actions" style="margin-top:0">
                    <button class="nbce-button secondary" data-action="pin-target" type="button">固定常用</button>
                    <button class="nbce-button secondary" data-action="clear-target-history" type="button">清空历史</button>
                  </div>
                </div>
                <div class="nbce-history-list"></div>
              </div>
            </div>
          ...
          <div class="nbce-pane nbce-debug-pane" data-pane="debug" hidden>
            ...
            <div class="nbce-debug-filters">
              <input class="nbce-debug-filter" data-filter="keyword" type="search" placeholder="搜索 endpoint / URL / 语义 / error / diagnostics / response" />
              <select class="nbce-debug-select" data-filter="method">
              ...
              <select class="nbce-debug-select" data-filter="statusFilter">
                <option value="all">全部状态</option>
                <option value="success">成功</option>
                <option value="failure">失败</option>
              </select>
              <select class="nbce-debug-select" data-filter="overrideFilter">
                <option value="all">全部改写</option>
                <option value="overridden">已改写</option>
                <option value="normal">未改写</option>
              </select>
              <label class="nbce-debug-filter-toggle" title="只看有诊断信息的请求">
                <input data-filter="diagnosticsOnly" type="checkbox" />
                <span>仅诊断</span>
              </label>
              <label class="nbce-debug-filter-toggle" title="只看资源问题，只展示只读资源问题分区">
                <input data-filter="resourceOnly" type="checkbox" />
                <span>仅资源问题</span>
              </label>
            </div>
            <div class="nbce-debug-summary"></div>
            <div class="nbce-debug-grid">
              ...
              <div class="nbce-debug-detail">
                ...
                <div class="nbce-debug-selected" hidden>
                  ...
                  <div class="nbce-actions">
                    <button class="nbce-button secondary" data-action="debug-copy-endpoint">复制 endpoint</button>
                    <button class="nbce-button secondary" data-action="debug-copy-semantic">复制语义摘要</button>
                    <button class="nbce-button secondary" data-action="debug-copy-diagnostics">复制诊断</button>
                    <button class="nbce-button secondary" data-action="debug-copy-package">复制完整调试包</button>
                ...
                <div class="nbce-debug-resource" hidden>
                  <div class="nbce-debug-resource-list"></div>
                  <div class="nbce-debug-empty-row">开启“仅资源问题”后，只显示只读资源问题分区，不展示请求详情编辑能力。</div>
                </div>
              </div>
            </div>
            <div class="nbce-note">
    ...
    const debugGrid = shadow.querySelector('.nbce-debug-grid');
    ...
    const debugSummary = shadow.querySelector('.nbce-debug-summary');
    const statusbarMain = shadow.querySelector('.nbce-statusbar-main');
    const statusbarSecondary = shadow.querySelector('.nbce-statusbar-secondary');
    const statusbarBadges = shadow.querySelector('.nbce-statusbar-badges');
    const historyList = shadow.querySelector('.nbce-history-list');
    const debugResource = shadow.querySelector('.nbce-debug-resource');
    const debugResourceList = shadow.querySelector('.nbce-debug-resource-list');
    const debugToggleButton = shadow.querySelector('[data-action="debug-toggle-logs"]');
    const filterElements = Array.from(shadow.querySelectorAll('[data-filter]'));
    ...
    function getActiveRuleForPanel() {
      return normalizeRule(rules[location.origin] || null) || state.lastRule || currentRule;
    }

    function getDebugViewState() {
      const debugRules = getDebugRulesForOrigin();
      const viewModels = buildDebugLogViewModels(debugLogs, {
        debugRules,
        analyzeRequest: analyzeNocoBaseRequest,
      });
      const requestResult = filterDebugLogViewModels(viewModels, state.debugFilters);
      const resourceResult = filterResourceIssues(resourceDiagnostics, state.debugFilters);
      const selection = resolveVisibleDebugSelection(
        state.selectedDebugLogId,
        requestResult.items,
        state.debugFilters.resourceOnly,
      );
      return {
        debugRules,
        viewModels,
        visibleItems: requestResult.items,
        visibleResourceIssues: resourceResult.items,
        counts: {
          ...requestResult.counts,
          ...resourceResult.counts,
        },
        selection,
      };
    }

    function getSelectedDebugLog(viewState = getDebugViewState()) {
      return viewState.selection.selectedLog || null;
    }
    ...
    function renderTargetHistory() {
      ...
      state.targetHistoryStore = loadTargetHistoryStore();
      const entry = getTargetHistoryForOrigin(location.origin, state.targetHistoryStore);
      const items = [
        ...(entry.pinned ? [{ url: entry.pinned, pinned: true }] : []),
        ...entry.recent.map((url) => ({ url, pinned: false })),
      ];
      if (!items.length) {
        historyList.innerHTML = '<div class="nbce-debug-empty-row">暂无最近使用目标。</div>';
        return;
      }
      historyList.innerHTML = items
        .map((item) => {
          const isActive = normalizeTargetHistoryUrl(state.targetUrl) === item.url;
          return `
            <div class="nbce-history-item">
              <button class="nbce-history-chip${item.pinned ? ' pinned' : ''}${isActive ? ' active' : ''}" data-target-url="${escapeHtml(
                item.url,
              )}" title="${escapeHtml(item.url)}" type="button">
                ${item.pinned ? '固定 · ' : ''}${escapeHtml(shortPanelText(item.url, item.url))}
              </button>
              <button class="nbce-icon-button" data-action="remove-target-history" data-target-url="${escapeHtml(
                item.url,
    ...
    function renderStatusbar(viewState) {
      const activeRule = getActiveRuleForPanel();
      const summary = buildPanelStatusSummary(activeRule, {
        debugEnabled: nbceDebugEnabled,
        isLikelyNocoBasePage: isProbablyNocoBasePage(),
      }, viewState.counts);
      if (statusbarMain) {
        statusbarMain.textContent = summary.main;
      }
      if (statusbarSecondary) {
        statusbarSecondary.textContent = summary.secondary;
      }
      if (statusbarBadges) {
        statusbarBadges.innerHTML = summary.badgeItems
          .map(
            (item) =>
              `<span class="nbce-statusbar-badge ${escapeHtml(item.tone)}">${escapeHtml(item.text)}</span>`,
          )
          .join('');
      }
      if (toggle) {
        toggle.setAttribute(
          'title',
          `${summary.badgeItems.map((item) => item.text).join(' · ') || 'NocoBase Cross Env'} · 点击展开，拖拽移动`,
        );
      }
    }
    ...
    function renderDebugFilters() {
      filterElements.forEach((element) => {
        ...
        if (element.type === 'checkbox') {
          element.checked = Boolean(state.debugFilters[key]);
          return;
        }
        element.value = state.debugFilters[key] || '';
      });
    }

    function renderDebugList(viewState) {
      if (!debugList) {
        return;
      }
      debugList.hidden = Boolean(state.debugFilters.resourceOnly);
      const items = viewState.visibleItems;
      if (state.debugFilters.resourceOnly) {
        debugList.innerHTML = '';
        return;
      }
      if (!items.length) {
        debugList.innerHTML = debugLogs.length
          ? '<div class="nbce-debug-empty">没有匹配当前筛选条件的请求。</div>'
          : '<div class="nbce-debug-empty">暂无桥接请求。</div>';
        return;
      }
      debugList.innerHTML = items
        .map((log) => {
          const selected = log.id === state.selectedDebugLogId ? ' active' : '';
          const rule = getDebugRuleForLog(log);
          const statusClass = log.isSuccessfulResponse ? 'ok' : 'error';
          const statusText = log.isTransportError ? 'ERR' : log.status || '-';
          const badges = [
            `<span class="nbce-badge ${statusClass}">${escapeHtml(statusText)}</span>`,
            log.isOverridden
              ? '<span class="nbce-badge mock">改写</span>'
              : rule
                ? '<span class="nbce-badge mock">有规则</span>'
                : '',
            Array.isArray(log.diagnostics) && log.diagnostics.length
              ? '<span class="nbce-badge error">诊断</span>'
              : '',
          ]
            .filter(Boolean)
            .join('');
    ...
    function renderResourceIssues(viewState) {
      if (!debugResource || !debugResourceList) {
        return;
      }
      debugResource.hidden = !state.debugFilters.resourceOnly;
      if (!state.debugFilters.resourceOnly) {
        debugResourceList.innerHTML = '';
        return;
      }
      const items = viewState.visibleResourceIssues;
      if (!items.length) {
        debugResourceList.innerHTML = '<div class="nbce-debug-empty-row">没有匹配当前筛选条件的资源问题。</div>';
        return;
      }
      debugResourceList.innerHTML = items
        .map(
          (issue) => `
            <div class="nbce-debug-resource-item">
              <div class="nbce-debug-resource-title">${escapeHtml(issue.level || 'warn')} · ${escapeHtml(
                issue.kind || 'resource',
              )}</div>
              <div class="nbce-debug-resource-body">${escapeHtml(
                [issue.message, issue.url ? `URL: ${issue.url}` : '', issue.detail ? `Detail: ${issue.detail}` : '']
                  .filter(Boolean)
                  .join('\n'),
              )}</div>
            </div>
    ...
    function renderDebugSummary(viewState) {
      if (!debugSummary) {
        return;
      }
      debugSummary.textContent = `请求 ${viewState.counts.requestHits}/${viewState.counts.requestTotal} · 资源问题 ${viewState.counts.resourceHits}/${viewState.counts.resourceTotal}`;
    }

    function syncDebugSelection(viewState) {
      const { selection } = viewState;
      if (selection.selectedDebugLogId !== state.selectedDebugLogId) {
        state.selectedDebugLogId = selection.selectedDebugLogId;
      }
      if (selection.shouldClearEditor) {
        state.debugEditorValue = '';
      } else if (selection.selectedLog && !state.debugEditorValue) {
        const rule = getDebugRuleForLog(selection.selectedLog);
        state.debugEditorValue = formatDebugJsonText(rule?.responseText || selection.selectedLog.responseText || '');
      }
      return selection.selectedLog;
    }

    function renderDebugDetail(viewState) {
      if (!debugEmpty || !debugSelected || !debugMeta || !debugTextarea || !debugStatus) {
        return;
      }
      const selectedLog = syncDebugSelection(viewState);
      const showResourceOnly = Boolean(state.debugFilters.resourceOnly);
      debugEmpty.hidden = Boolean(selectedLog) || showResourceOnly;
      debugSelected.hidden = !selectedLog || showResourceOnly;
      debugResource.hidden = !showResourceOnly;
      if (!selectedLog) {
        debugEmpty.textContent = debugLogs.length
          ? '请选择左侧可见请求，或调整筛选条件。'
          : '暂无请求。触发一次被桥接的 NocoBase API 后会显示在这里。';
        debugStatus.textContent = '';
        renderResourceIssues(viewState);
        return;
      }
      ...
      const semantic = {
        summary: selectedLog.semanticSummary,
        filterText: selectedLog.semanticFilterText,
      };
      ...
      const metaRows = [
        ...
        ['Semantic', selectedLog.semanticSummary || '未识别到 NocoBase collection/action 语义'],
        [
          'Status',
          selectedLog.isSuccessfulResponse
            ? `${selectedLog.status || 0} ${selectedLog.statusText || ''}`.trim()
            : selectedLog.isHttpError
              ? `HTTP ${selectedLog.status || 0} ${selectedLog.statusText || ''}`.trim()
              : `请求失败${selectedLog.transportReason ? `：${selectedLog.transportReason}` : ''}`,
        ],
        ['Duration', `${selectedLog.durationMs} ms`],
        ['Override', selectedLog.isOverridden ? '已命中本地改写' : rule ? '已有本地改写规则' : '无'],
        ['Diagnostics', formatDebugDiagnostics(requestDiagnostics)],
        ['URL', formatDebugUrlPath(selectedLog.url)],
      ];
      if (recentResourceIssues.length) {
        metaRows.push(['Recent Resource Issues', formatDebugDiagnostics(recentResourceIssues)]);
      }
      if (queryText) {
      ...
      if (selectedLog.requestBodySummary) {
        ...
        if (selectedLog.requestBodyMeta?.truncated) {
          metaRows.push(['Body Meta', `truncated: true\nomittedChars: ${selectedLog.requestBodyMeta.omittedChars || 0}`]);
        }
      }
    ...
    function renderDebug() {
      ...
      const viewState = getDebugViewState();
      syncDebugSelection(viewState);
      debugGrid?.classList.toggle('resource-only', Boolean(state.debugFilters.resourceOnly));
      if (debugToggleButton) {
        debugToggleButton.textContent = state.debugEnabled ? '关闭详细日志' : '开启详细日志';
      }
      renderDebugFilters();
      renderDebugSummary(viewState);
      renderDebugList(viewState);
      renderDebugDetail(viewState);
      renderStatusbar(viewState);
      return viewState;
    }
    ...
    filterElements.forEach((element) => {
      ...
      element.addEventListener(eventName, () => {
        state.debugFilters[key] = element.type === 'checkbox' ? Boolean(element.checked) : element.value;
        render();
      });
    });
    ...
    shadow.addEventListener('click', async (event) => {
      const removeHistoryButton = event.target.closest('[data-action="remove-target-history"]');
      if (removeHistoryButton && historyList?.contains(removeHistoryButton)) {
        const targetUrl = removeHistoryButton.getAttribute('data-target-url') || '';
        state.targetHistoryStore = await updateTargetHistoryStore((store) =>
          removeTargetHistory(store, location.origin, targetUrl),
        );
        render();
        return;
      }

      const targetHistoryChip = event.target.closest('[data-target-url]');
      if (targetHistoryChip && historyList?.contains(targetHistoryChip)) {
        const targetUrl = targetHistoryChip.getAttribute('data-target-url') || '';
      ...
      if (action === 'pin-target') {
        ...
        state.targetHistoryStore = await updateTargetHistoryStore((store) =>
          pinTargetHistory(store, location.origin, state.targetUrl),
        );
        state.status = '已固定当前 B 为常用目标。';
        render();
        return;
      }

      if (action === 'clear-target-history') {
        state.targetHistoryStore = await updateTargetHistoryStore((store) => {
          const nextStore = normalizeTargetHistory(store);
          delete nextStore[location.origin];
          return nextStore;
        });
        state.status = '已清空当前 A 域名下的最近使用目标。';
        render();
        return;
      }

      if (
        action === 'debug-copy-url' ||
        action === 'debug-copy-response' ||
        action === 'debug-copy-endpoint' ||
        action === 'debug-copy-semantic' ||
        action === 'debug-copy-diagnostics' ||
        action === 'debug-copy-package'
      ) {
        const selectedLog = getSelectedDebugLog();
        if (!selectedLog) {
          state.debugStatus = '请先选择一条请求。';
        ...
        try {
          if (action === 'debug-copy-url') {
            await copyDebugText(selectedLog.url);
            state.debugStatus = '已复制 URL。';
          } else if (action === 'debug-copy-response') {
            await copyDebugText(state.debugEditorValue);
            state.debugStatus = '已复制响应内容。';
          } else if (action === 'debug-copy-endpoint') {
            await copyDebugText(getDebugCopyPayloadText(selectedLog, 'endpoint'));
            state.debugStatus = '已复制 endpoint。';
          } else if (action === 'debug-copy-semantic') {
            await copyDebugText(getDebugCopyPayloadText(selectedLog, 'semantic'));
            state.debugStatus = '已复制语义摘要。';
          } else if (action === 'debug-copy-diagnostics') {
            await copyDebugText(getDebugCopyPayloadText(selectedLog, 'diagnostics'));
            state.debugStatus = '已复制诊断。';
          } else {
            await copyDebugText(
              getDebugCopyPayloadText(selectedLog, 'package', {
                editorValue: state.debugEditorValue,
                semantic: {
                  summary: selectedLog.semanticSummary,
                  filterText: selectedLog.semanticFilterText,
                },
                diagnostics: selectedLog.diagnostics,
                overrideRule: getDebugRuleForLog(selectedLog),
              }),
            );
            state.debugStatus = '已复制完整调试包。';
          }
        } catch (error) {
      ...
      if (action === 'save') {
        ...
        try {
          ...
          state.targetHistoryStore = await updateTargetHistoryStore((store) =>
            upsertTargetHistory(store, location.origin, parsedRule.targetEntryUrl),
          );
  ...
  if (typeof globalThis !== 'undefined' && globalThis.__NBCE_EXPOSE_INTERNALS_FOR_TEST__) {
    globalThis.__NBCE_TEST_INTERNALS__ = {
      ...
      addDebugLog,
      addResourceDiagnostic,
      summarizeDebugBodyMeta,
      deriveDebugLogStatus,
      buildDebugLogViewModels,
      filterDebugLogViewModels,
      filterResourceIssues,
